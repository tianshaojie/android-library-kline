...
public class MinuteChartView extends View implements GestureDetector.OnGestureListener {
    ...
    private final List<MinuteLine> mPoints = new ArrayList<>();
    ...
    {
        Date date=mPoints.get(position).time;
     ...
     */
    public void initData(Collection<? extends MinuteLine> data,
                         Date startTime,
                         Date endTime,
                         float yesClosePrice) {
    ...
    public void notifyChanged() {
        ...
        for (int i = 0; i < mPoints.size(); i++) {
            MinuteLine point = mPoints.get(i);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawGird(canvas);
        if (mPoints.size() > 0) {
            MinuteLine lastPoint = mPoints.get(0);
            float lastX=getX(0);
            for (int i = 0; i < mPoints.size(); i++) {
                MinuteLine curPoint=mPoints.get(i);
                float curX=getX(i);
                canvas.drawLine(lastX, getY(lastPoint.price), curX, getY(curPoint.price), mPricePaint);
                canvas.drawLine(lastX, getY(lastPoint.avgPrice),curX, getY(curPoint.avgPrice), mAvgPaint);
                //成交量
                Paint volumePaint=((i==0&&curPoint.price<=mValueStart)||curPoint.price<=lastPoint.price)?mVolumePaintGreen:mVolumePaintRed;
                canvas.drawLine(curX,getVolumeY(0),curX,getVolumeY(curPoint.volume),volumePaint);
                lastPoint = curPoint;
                lastX=curX;
            }
        }
        ...
        //画指示线
        if (isLongPress) {
            MinuteLine point = mPoints.get(selectedIndex);
            float x=getX(selectedIndex);
            canvas.drawLine(x, 0, x, mHeight+mVolumeHeight, mTextPaint);
            canvas.drawLine(0, getY(point.price), mWidth, getY(point.price), mTextPaint);
            //画指示线的时间
            String text = DateUtil.shortTimeFormat.format(point.time);
    ...
    private void drawValue(Canvas canvas, int index) {
        ...
        if (index >= 0 && index < mPoints.size()) {
            ...
            MinuteLine point = mPoints.get(index);
            String text = "成交价:" + floatToString(point.price) + " ";
            float x = 0;
            canvas.drawText(text, x, y, mPricePaint);
            x += mPricePaint.measureText(text);
            text = "均价:" + floatToString(point.avgPrice) + " ";
     ...
     * @param position 索引值
     */
    public void changePoint(int position,MinuteLine point)
    {
     ...
     * 刷新最后一个点
     */
    public void refreshLastPoint(MinuteLine point) {
     ...
     * 添加一个点
     */
    public void addPoint(MinuteLine point) {
        mPoints.add(point);
        notifyChanged();
    }
     ...
     * 根据索引获取点
     */
    public MinuteLine getItem(int position)
    {
